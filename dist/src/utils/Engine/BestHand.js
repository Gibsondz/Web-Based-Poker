...
class BestHand {
    ...
    findWinner() {
        ...
        if (bestHands[0][2].categoryName === ("High Card" /* HC */ || "Straight" /* ST */ || "Flush" /* FL */ || "Straight Flush" /* SF */)) {
        ...
        if (bestHands[0][2].categoryName === "One Pair" /* OP */) {
            let largestHands = [bestHands[0]];
        ...
        if (bestHands[0][2].categoryName === "Two Pair" /* TP */) {
            let largestHands = [bestHands[0]];
        ...
        if (bestHands[0][2].categoryName === ("Three of a Kind" /* TK */ || "Four of a Kind" /* FK */)) {
            let largestHands = [bestHands[0]];
            ...
            for (let i = 1; i < bestHands.length; i++) {
                if (bestHands[i][1][2] > player_maxPair[1]) {
                    ...
                    let challengingHC;
                    let currentHC;
                    if (bestHands[0][2].categoryName === "Four of a Kind" /* FK */) {
                        ...
                        challengingHC = bestHands[i][1][0] === bestHands[i][1][2] ? (bestHands[i][1][1] === bestHands[i][1][2] ? bestHands[i][1][3] : bestHands[i][1][1]) : bestHands[i][1][0];
                        currentHC = bestHands[bestIdx][1][0] === player_maxPair[1] ? (bestHands[bestIdx][1][1] === player_maxPair[1] ? bestHands[bestIdx][1][3] : bestHands[bestIdx][1][1]) : bestHands[bestIdx][1][0];
                    }
                    if (challengingHC > currentHC) {
                        player_maxPair[0] = i;
                        largestHands = [];
        ...
        if (bestHands[0][2].categoryName === "Full House" /* FH */) {
            let largestHands = [bestHands[0]];
    ...
    findBestHand(possibleCards) {
        ...
        if (tiedCardArray.length === 1)
            return [tiedCardArray[0], BestHand.CategoryList[bestIndex]];
...
*/
    generateCombinations(sourceArray, comboLength) {
        ...
        if (comboLength > sourceLength)
            return [];
        const combos = new Array(); // Stores valid combinations as they are generated.
...
BestHand.CategoryList = [
    { categoryName: "Four of a Kind" /* FK */, rank: 7 },
    { categoryName: "Straight Flush" /* SF */, rank: 8 },
    { categoryName: "Straight" /* ST */, rank: 4 },
    { categoryName: "Flush" /* FL */, rank: 5 },
    { categoryName: "High Card" /* HC */, rank: 0 },
    { categoryName: "One Pair" /* OP */, rank: 1 },
    { categoryName: "Two Pair" /* TP */, rank: 2 },
    { categoryName: "Royal Flush" /* RF */, rank: 9 },
    { categoryName: "Three of a Kind" /* TK */, rank: 3 },
    { categoryName: "Full House" /* FH */, rank: 6 },
];
